...
byh.initialize = function () {
    byh.listenForEvent("byh:jinrushijie", function () {
        ...
        byh.listenForEvent("byh:jinrushijie1_kg", jinrushijie1/* function () {            
    ...
    function log_record () {
        ...
        */
        ...
        //自带事件
        byh.listenForEvent("minecraft:block_interacted_with", (eventData) =>logrecords.hudong(eventData))
        byh.listenForEvent("minecraft:block_destruction_started", (eventData) =>logrecords.kaishipohuai(eventData));
        byh.listenForEvent("minecraft:block_destruction_stopped", (eventData) =>logrecords.tingzhipohuai(eventData));
        byh.listenForEvent("minecraft:player_placed_block", (eventData) =>logrecords.fangzhi(eventData));    
        byh.listenForEvent("minecraft:player_destroyed_block", (eventData) =>logrecords.pohuai(eventData))
        byh.listenForEvent("minecraft:piston_moved_block", (eventData) =>logrecords.huoshai(eventData));
        byh.listenForEvent("minecraft:entity_carried_item_changed", (eventData) =>logrecords.shouchiwu(eventData)); 
        byh.listenForEvent("minecraft:entity_dropped_item", (eventData) =>logrecords.diuchuwuping(eventData));
        byh.listenForEvent("minecraft:entity_acquired_item", (eventData) =>logrecords.huodewuping(eventData));
        byh.listenForEvent("minecraft:entity_equipped_armor", (eventData) =>logrecords.zhuangjia(eventData));
        byh.listenForEvent("minecraft:entity_use_item", (eventData) =>logrecords.shiyongwuping(eventData));
        byh.listenForEvent("minecraft:entity_death", (eventData) =>logrecords.siwang(eventData));
        byh.listenForEvent("minecraft:entity_start_riding", (eventData) =>logrecords.qicheng(eventData));
        byh.listenForEvent("minecraft:entity_start_riding", (eventData) =>logrecords.tingzhiqicheng(eventData));
        byh.listenForEvent("minecraft:block_exploded", (eventData) =>logrecords.baozha(eventData));
        byh.listenForEvent("minecraft:entity_sneak", (eventData) =>logrecords.xingzou(eventData));
        byh.listenForEvent("minecraft:entity_attack", (eventData) =>logrecords.gongji(eventData));
        byh.listenForEvent("minecraft:projectile_hit", (eventData) =>logrecords.jizhong(eventData));
        byh.listenForEvent("minecraft:entity_hurt", (eventData) =>logrecords.shanghai(eventData));
        byh.listenForEvent("minecraft:weather_changed", (eventData) =>logrecords.tianqi(eventData));
        byh.listenForEvent("minecraft:entity_definition_event", (eventData) =>logrecords.shijian(eventData));
...
function chatf (v,n,c) {
    ...
    if (v != null && n == null && c == null) {        
        ...
        eventData.data.command = 'tellraw @a { \"rawtext\" : [ { \"text\" : \"' + v + '\" } ] }';
        byh.broadcastEvent("minecraft:execute_command", eventData);   
    } else if (v != null && n != null && c == null ) {
        var eventData = byh.createEventData("minecraft:execute_command");
        eventData.data.command = 'tellraw @a { \"rawtext\" : [ { \"text\" : \"<' + n + '>' + v + '§r\" } ] }';
        byh.broadcastEvent("minecraft:execute_command", eventData);
    } else if (v != null && n == null && c != null) {
        var eventData = byh.createEventData("minecraft:execute_command");
        eventData.data.command = 'tellraw @a { \"rawtext\" : [ { \"text\" : \"§' + c1 + v + '\" } ] }';
        byh.broadcastEvent("minecraft:execute_command", eventData);
    } else if (v != null && n != null && c !== null) {
        var eventData = byh.createEventData("minecraft:execute_command");
        eventData.data.command = 'tellraw @a { \"rawtext\" : [ { \"text\" : \"<' + n + '>§' + c1 + v + '§r\" } ] }';
        byh.broadcastEvent("minecraft:execute_command", eventData);
    }
...
const logrecords = {
    hudong: function (event) {//*
        ...
        var neirong = 'Server: ' + player + ' 在 ' + x  + ', ' + y + ', ' + z + ' 处互动方块';
    ...
    kaishipohuai: function (event) {//*
        ...
        var neirong = 'Server: ' + player + ' 在 ' + x + ', ' + y + ', ' + z + ' 处开始破坏方块';
        chatf(neirong,'Log_record');
        log(neirong);
    },
    ...
    tingzhipohuai: function (event) {//*
        ...
        var neirong = 'Server: ' + player + ' 在 ' + x + ', ' + y + ', ' + z + ' 处停止破坏方块 破坏进度:' + jindu;
        chatf(neirong,'Log_record');
        log(neirong);
    },
    ...
    fangzhi: function (event) {//*
        ...
        var neirong = 'Server: ' + player + ' 在 ' + x + ', ' + y + ', ' + z + ' 处放置了一个方块'; 
        chatf(neirong,'Log_record');
        log(neirong);
    ...
    pohuai: function (event) {//*
        ...
        var neirong = 'Server: ' + player + ' 在 ' + x + ', ' + y + ', ' + z + ' 处 破坏了 ' + blockid;
        chatf(neirong,'Log_record');
        log(neirong);
    },
    ...
    huoshai: function (event) {//*
        if (event.data.piston_action == 'extended') {
            if (event.data.block_position != undefined) {
                ...
                var neirong = 'Server: ' + x1 + ', ' + y1 + ', ' + z1 + ' 处的活塞 ' + '推动(' + dongzuo + ') 了 ' + x  + ', ' + y + ', ' + z + ' 处的方块';
                chatf(neirong,'Log_record');
                log(neirong);       
            } else {
                x1 = event.data.piston_position.x;
                ...
                var neirong = 'Server: ' + x1 + ', ' + y1 + ', ' + z1 + ' 处的活塞 ' + '推动(' + dongzuo + ') 了 ';
                chatf(neirong,'Log_record');
                log(neirong);  
            }
        } else {
            if (event.data.block_position != undefined) {
                ...
                var neirong = 'Server: ' + x1 + ', ' + y1 + ', ' + z1 + ' 处的活塞 ' + '缩回(' + dongzuo + ') 了 ' + x  + ', ' + y + ', ' + z + ' 处的方块';
                chatf(neirong,'Log_record');
                log(neirong);       
            } else {
                x1 = event.data.piston_position.x;
                ...
                var neirong = 'Server: ' + x1 + ', ' + y1 + ', ' + z1 + ' 处的活塞 ' + '缩回(' + dongzuo + ') 了 ';
                chatf(neirong,'Log_record');
                log(neirong);  
            }
        }
    ...
    shouchiwu: function (event) {//*
        if (event.data.previous_carried_item.item != 'minecraft:undefined' && event.data.carried_item.item != 'minecraft:undefined') {
            ...
            var neirong = 'Server: ' + entity + ' 的 ' + shou + ' 的手持物从 ' + zhiqian + ' 改变成了 ' + xianzai;
            chatf(neirong,'Log_record');
            log(neirong);
            ...
            var neirong = 'Server: ' + entity + ' 的 ' + shou  + ' 的手持物从 ' + 'minecraft:air' + ' 改变成了 ' + xianzai;
            chatf(neirong,'Log_record');
            log(neirong);
        } else {
            entity = event.data.entity.__identifier__;
            ...
            var neirong = 'Server: ' + entity + ' 的 ' + shou  + ' 的手持物从 ' + zhiqian + ' 改变成了 ' + 'minecraft:air';
            chatf(neirong,'Log_record');
            log(neirong);
        }
    },
    ...
    huodewuping: function (event) {//* 4/5
        if (event.data.item_stack.item != 'minecraft:undefined') {
            item = event.data.item_stack.item;
            entity = event.data.entity.__identifier__;
            ...
            var neirong = 'Server: ' + entity + ' 获得了 ' + shuliang + ' 个 ' + item + ' 方式:' + fangshi;   
            chatf(neirong,'Log_record');
            log(neirong);
        } else {
            entity = event.data.entity.__identifier__;
    ...
    zhuangjia: function (event) {//*
        if (event.data.item_stack.item != 'minecraft:undefined') {
            ...
            var neirong = 'Server: ' + entity + ' 在 ' + caowei + '槽位 装上了 ' + item + ' 装备';   
            chatf(neirong,'Log_record');
            log(neirong);
        } else {
            caowei = event.data.slot;
    ...
    shiyongwuping: function (event) {//*
        ...
        var neirong = 'Server: ' + entity + " 正在使用 " + item + " 使用方式: " + fangshi;
        chatf(neirong,'Log_record');
        log(neirong);
    },

    siwang: function (event) {//* 4/5
        if (event.data.killer != undefined) {
            ...
            var neirong = 'Server: ' + entity + ' 被 ' + shashou + ' 杀死了 死因: ' + yuanyin + ' 弹射物:' + tanshewu;
            chatf(neirong,'Log_record');
            log(neirong);
        } else {
            yuanyin = event.data.cause;
    ...
    tingzhiqicheng: function (event) {//*
        if (event.data.entity_is_being_destroyed == true) {
            ...
            var neirong = 'Server: ' + zhu + ' 停止了对实体进行骑乘 原因: ' + '被骑者已经死亡' + '(' + yuanyin + ')';
            chatf(neirong,'Log_record');
            log(neirong);
            ...
            var neirong = 'Server: ' + zhu + ' 停止了对实体进行骑乘 原因: ' + '骑手自行决定停止骑乘' + '(' + yuanyin + ')';
            chatf(neirong,'Log_record');
            log(neirong);
        } else {
            zhu = event.data.entity.__identifier__;
            ...
            var neirong = 'Server: ' + zhu + ' 停止了对实体进行骑乘 原因: ' + '骑手正在骑另一个实体' + '(' + yuanyin + ')';
            chatf(neirong,'Log_record');
            log(neirong);
        }
    },
    ...
    baozha: function  (event) {//*
        ...
        var neirong = 'Server: ' + block + ' 在 ' + x + ', ' + y + ', ' + z + ' 处 因为 ' + yuanyin + ' 被 ' + entity + '破坏';
        chatf(neirong,'Log_record');
        log(neirong);
    },
    ...
    xingzou: function (event) {//*
        if (event.data.sneaking == true) {
            ...
            var neirong = 'Server: ' + entity + ' 退出了 潜行' + '(' + zhuangtai +')';
            chatf(neirong,'Log_record');
            log(neirong);
        }
    },
    ...
    jizhong: function  (event) {//*
        ...
        if (entity != null){
            entity = event.data.entity.__identifier__;
            var neirong = 'Server: ' + fashezhe + ' 在 '+ zuobiao + ' 发射的 ' + fashewu + ' 击中了 ' + entity;
            chatf(neirong,'Log_record');
            log(neirong);
        } else {                       
            var neirong = 'Server: ' + fashezhe + ' 在 '+ zuobiao + ' 发射了 ' + fashewu;
            chatf(neirong,'Log_record');
            log(neirong);
        }
    },

    shanghai: function (event) {//* 5/7
        entity = event.data.entity.__identifier__;
        ...
        if (laiyuan != null) {
            ...
            var neirong = 'Server: ' + entity + ' 受到了 ' + laiyuan + ' 的 ' + shiji + ' 点伤害 方式:' + fangshi + ' 实体吸收伤害:' + xishou;
            chatf(neirong,'Log_record');
            log(neirong);
        } else {
            var neirong = 'Server: ' + entity + ' 受到了 ' + shiji + ' 点伤害 方式:' + fangshi + ' 实体吸收伤害:' + xishou;
            chatf(neirong,'Log_record');
            log(neirong);
        }
    },
    ...
    yinyue: function (event) {//*
        ...
        var neirong = 'Server: ' + zuobiao + ' 正在播放 ' + yinyueid + ' 音高:' + yingao + ' 音量' + yinliang;
        chatf(neirong,'Log_record');
        log(neirong);
    },
...
function jinrushijie1 (eventData) {
    ...
    if (ulang_qianzhi == true) {
        var neirong = '§l§7[BYH]ULANG框架Addon 已成功加载\n[BYH]当前ULANG框架版本：' + ulang_banben + '\n§7[BYH]BY 碧月狐dada \n§7[BYH]当前被内嵌Addon：' + neiqian_addon + " 被内嵌Addon版本：" + neiqian_banben + '\n§7[BYH]BY 碧月狐dada'     
...
function jinrushijie2 () {
    var neirong = 'tellraw @a { \"rawtext\" : [ { \"text\" : \"§6按 T键 或 回车 以打开聊天栏查看更多内容\" } ] }';
